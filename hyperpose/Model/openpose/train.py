...
from utils.gcloud_utils import *
...
def _map_fn(img_dat, annos ,data_aug_fn, hin, win, hout, wout, parts, limbs, onTPU=False):
    """TF Dataset pipeline."""
    #load data
    if not onTPU:
        img_dat = tf.io.read_file(img_dat)
    image = tf.image.decode_jpeg(img_dat, channels=3)  # get RGB with 0~1
...
def get_paramed_map_fn(hin,win,hout,wout,parts,limbs,flip_list=None,data_format="channels_first", onTPU=False):
    ...
    paramed_map_fn=partial(_map_fn,data_aug_fn=paramed_data_aug_fn, hin=hin, win=win, hout=hout, wout=wout ,parts=parts, limbs=limbs, onTPU=onTPU)
...
def tpu_train(train_model, dataset, config):
    '''TPU train pipeline of Openpose class models
    ...
    print(f"TPU training using learning rate:{lr_init} batch_size:{batch_size}")
    # training dataset configure with shuffle,augmentation,and prefetch
    ...
    paramed_map_fn = get_paramed_map_fn(hin, win, hout, wout, parts, limbs, flip_list=flip_list,
                                        data_format=data_format, onTPU=True)
    train_dataset = train_dataset.shuffle(buffer_size=4096).repeat()
    train_dataset = train_dataset.map(paramed_map_fn, num_parallel_calls=max(multiprocessing.cpu_count() // 2, 1))
    train_dataset = train_dataset.batch(config.train.batch_size)
    train_dataset = train_dataset.prefetch(64)
    ...
    @tf.function
    def distributed_train_step(image, gt_label, mask, train_model):
        per_replica_losses = strategy.run(one_step, args=(image, gt_label, mask, train_model))
    ...
    for image, gt_label, mask in train_dataset:
        ...
        # optimize one step
        """
        gt_conf, gt_paf, pd_conf, pd_paf, total_loss, re_loss, loss_confs, loss_pafs = one_step(image.numpy(),
                                                                                                gt_label.numpy(),
                                                                                                mask.numpy(),
                                                                                                train_model)
        """
        global_total_loss += distributed_train_step(image.numpy(), gt_label.numpy(), mask.numpy(), train_model)
        # debug
        # for stage_id,(loss_conf,loss_paf) in enumerate(zip(loss_confs,loss_pafs)):
        #    print(f"test stage:{stage_id} loss_conf:{loss_conf} loss_paf:{loss_paf}")
        ...
        if ((step != 0) and (step % save_interval) == 0):
            ...
            log(f"model save_path:{model_save_path} saved!\n")
        """
        print("step:{}, loss:{}".format(step.numpy(), global_total_loss))
        # training finished
        if (step == n_step):
            break
